...
// myAudioHandler.
    final player = ConcatenatingAudioSource(children: [
      ...
      if (!isPlayedNavinfo)
        AudioSource.uri(Uri.parse('asset:///assets/navinfo.mp3')),
      AudioSource.uri(Uri.file(widget.bookNodes[selectedItem].tts))
    ]);
    _player.setAudioSource(player);
    ...
    return Scaffold(
      body: GestureDetector(
          onTap: () async {
            ...
            // await _player.setUrl(widget.bookNodes[selectedItem].tts);
          ...
          onDoubleTap: () async {
            ...
            if (widget.bookNodes[selectedItem].isCategory)
              await Navigator.push(
                  context,
                  MaterialPageRoute(
                      builder: (context) => SelectorPageTts(
                          bookNodes: widget.bookNodes[selectedItem].children)));
            else
              await Navigator.push(
                  context,
                  MaterialPageRoute(
                      builder: (context) =>
                          PlayerPageTts(widget.bookNodes[selectedItem])));
            final player = ConcatenatingAudioSource(children: [
              AudioSource.uri(Uri.parse('asset:///assets/categories.mp3')),
              AudioSource.uri(Uri.file(widget.bookNodes[selectedItem].tts))
            ]);
          await  _player.setAudioSource(player);
            ...
            // _player.load(ConcatenatingAudioSource(children: [
            //   AudioSource.uri(Uri.parse('asset:///assets/categories.mp3')),
            //   AudioSource.uri(Uri.file(widget.bookNodes[selectedItem].tts))
            // ]));
            await _player.play();
    ...
    return Padding(
        ...
        child: Column(
          children: [
            Expanded(
                child: Row(children: [
              Expanded(child: myGridI(offset + 0)),
              Expanded(child: myGridI(offset + 1))
            ])),
            Expanded(
                child: Row(children: [
              Expanded(child: myGridI(offset + 2)),
              Expanded(child: myGridI(offset + 3))
            ])),
            Expanded(
                child: Row(children: [
              Expanded(child: myGridI(offset + 4)),
              Expanded(child: myGridI(offset + 5))
            ])),
          ],
        ));
  }
