          ...
          Expanded(
            ...
            onDoubleTap: () async {
              if (ttsMode) {
                ...
                final play = ConcatenatingAudioSource(children: [
                  AudioSource.uri(Uri.parse('asset:///assets/homepage.mp3')),
                  AudioSource.uri(
                      Uri.parse('asset:///assets/entertocategories.mp3'))
                ]);
                await plyr.setAudioSource(play);
                // await plyr.setAudioSource(
                //     AudioSource.uri(Uri.parse('asset:///assets/homepage.mp3')));
                // await plyr.setAudioSource(AudioSource.uri(
                //     Uri.parse('asset:///assets/entertocategories.mp3')));
...
class _TtsDownloaderState extends State<TtsDownloader> {
  ...
  Future<void> doIt() async {
    ...
    try {
      ...
      var ttsRes =
          await http.get(Uri.parse("https://danisankitab.az/tts.json"));
